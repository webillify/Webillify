...
| `WBL-RISK-002` | Risk           | HIGH     | Angular still uses demo auth despite secure identity API being ready | BE-003 passes; frontend API adapter pending       | Wire API adapter and verify deployed cookie/CSRF behavior | Full stack           | MITIGATED API — UI OPEN          |
...
| `WBL-RISK-004` | Risk           | HIGH     | Full-stack regression coverage remains incomplete                    | Frontend 19+6 and API 3+15 tests pass              | Expand QA-004 as transaction modules land                 | QA/backend           | MITIGATING                       |
